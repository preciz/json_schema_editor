defmodule JSONSchemaEditor.ProgressiveDisclosureTest do
  ...
  test "highlights constraints icon when data is present" do
    scenarios = [
      ...
      {%{"type" => "object", "additionalProperties" => false}, "jse-btn-toggle-constraints.*jse-has-data"},
      {%{"type" => "array", "uniqueItems" => true}, "jse-btn-toggle-constraints.*jse-has-data"},
      {%{"type" => "string"}, "jse-btn-toggle-constraints"} # Should NOT have jse-has-data (this is a bit weak as regex)
    ]
  ...
  test "highlights logic icon when data is present" do
    scenarios = [
      {%{"type" => "string", "if" => %{"type" => "string"}}, "jse-btn-toggle-logic.*jse-has-data"},
      {%{"type" => "string", "not" => %{"type" => "number"}}, "jse-btn-toggle-logic.*jse-has-data"},
      {%{"type" => "string"}, "jse-btn-toggle-logic"} # Should NOT have jse-has-data
    ]
  ...
  test "auto-expands node when toggling constraints/logic/description" do
    ...
    {:noreply, socket} = JSONSchemaEditor.handle_event(
      "toggle_ui", 
