defmodule JSONSchemaEditor.SchemaMutatorPropertyTest do
  ...
  property "change_type correctly resets fields for the target type" do
    check all(
            properties <-
              map_of(string(:alphanumeric, min_length: 1, max_length: 10), simple_schema(),
                min_length: 1,
                max_length: 10
              ),
  ...
  property "toggle_required is its own inverse" do
    check all(
      ...
      clean_before =
        if Map.get(node_before, "required") == nil,
          do: Map.put(node_before, "required", []),
          else: node_before
  ...
  property "add_property followed by delete_property returns to original (mostly)" do
    check all(
      ...
      clean_before =
        if Map.get(node_before, "required") in [nil, []],
          do: Map.delete(node_before, "required"),
          else: node_before

      clean_after =
        if Map.get(node_after, "required") in [nil, []],
          do: Map.delete(node_after, "required"),
          else: node_after
